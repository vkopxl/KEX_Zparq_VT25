...
class CANInterface:
    ...
    def read_data(self, timeout=10.0, use_smoothing = True):
    ...
    # Applicerar scaling och offsets.
        while time.time() - start_time < timeout:
            ...
            if message:
                if message.arbitration_id == 0x0CC6D4E1:  # Inverter Status
                    ...
                    varvtal = int.from_bytes(message.data[2:4], byteorder='little') * -1 * 0.5 - 16000
        ...
        if use_smoothing:
            ...
            filtered_vridmoment = vridmoment
            filtered_varvtal = varvtal
            filtered_speed = speed

        # P = M * 2pi*RPM / 60
        power = filtered_vridmoment * (2 * np.pi * filtered_varvtal / 60)

        return tilt_percent, power, filtered_speed, filtered_varvtal, filtered_vridmoment
    ...
    def read_only_mode(self):
        try:
            while True:
                tilt_percent, power, speed, filtered_varvtal, filtered_vridmoment = self.read_data(use_smoothing = False)
                ...
                if None not in [tilt_percent, power, speed]:
                    ...
                    print(f"Tilt: {tilt_percent / 100:.2f}% | "
                        ...
                        f"Effekt: {power:.2f} W | "
                        f"Vridmoment: {filtered_vridmoment:.2f} Nm | "
                        f"Varvtal: {filtered_varvtal:.2f} RPM | "
...
def main():
    print("Välj algorithm: '1': Hill climbing, '2': Gradient ascent, '3': READ ONLY Mode")
    val = input()
    ...
    if val == '3':
        ...
        print("Displaying data received from CAN-bus")
    ...
    try:
        if val == '1':
            ...
            print("Algoritm vald: GRADIENT ASCENT")

...
class Lookuptable:
# sparar och hämtar data ur LUTs csv
    def __init__(self, filename="lookuptable.csv"):
        self.filename = filename
        self.data = []
        
    
        speed, torque, rpm = self.can.read_data()


    def open_luts():
        # Example: saving the lookup table to a CSV file
        with open('tilt_lookup.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            # Write a header row for clarity
            writer.writerow(["speed", "rpm", "torque", "tilt"])
            for entry in lookup_table:
                writer.writerow([entry["speed"], entry["rpm"], entry["torque"], entry["tilt"]])

        # Example: loading the lookup table from a CSV file
        loaded_table = []
        with open('tilt_lookup.csv', 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Convert string fields to float (assuming all fields are numeric)
                loaded_table.append({

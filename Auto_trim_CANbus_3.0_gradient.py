...
class TrimAlgorithm:
    def __init__(self, can_interface, csv_logger, alpha=500, tolerance=0.01, max_iterations=100):
    ...
    def compute_gradient(self, tilt1, eff1, tilt2, eff2):
        # Approximerar gradienten för effektiviteten 
        return (eff2 - eff1) / (tilt2 - tilt1) if tilt2 != tilt1 else 0
    ...
    def run(self):
        ...
        # Behöver finnas en första gång här!!!!?
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            # Testa en liten ändring i tilt för att beräkna gradient
            delta_tilt = 500  # Litet teststeg
            test_tilt = self.can.clamp_tilt_percent(current_tilt + delta_tilt)
            self.can.current_tilt_percent = test_tilt

            # Vänta på stabilisering
            time.sleep(3)

            _, power_test, speed_test = self.can.read_data()
            test_efficiency = self.measure_efficiency(speed_test, power_test)

            # Beräkna gradienten
            gradient = self.compute_gradient(current_tilt, prev_efficiency, test_tilt, test_efficiency)

            # Uppdatera tilt baserat på gradienten
            new_tilt = self.can.clamp_tilt_percent(current_tilt + self.alpha * gradient)
            ...
            new_efficiency = self.measure_efficiency(power,speed)
            ...
            # Kontrollera konvergens (gradient nära noll)
            ...
            current_tilt, prev_efficiency = new_tilt, new_efficiency

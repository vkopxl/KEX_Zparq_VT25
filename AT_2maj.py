...
# Exponential Moving Average (EMA) filter för att hantera brus
    def __init__(self, base_alpha=1, max_alpha=1):
...
# Trim-algoritm som optimerar tilt för bästa effektivitet (Gradient Ascent), har alternativet momentum gradient ascent.
    def __init__(self, can_interface, csv_logger, step_size= -200, tolerance=0.0005, max_iterations=100, alpha= 100000, beta=0.98, v=0, use_momentum=False, fake = False, sim = False):
    ...
    def measure_efficiency(self, speed, power):
        return 1000 * speed / power if power else 0
...
# Trim-algoritm som optimerar tilt för bästa effektivitet (Hill Climbing)
    def __init__(self, can_interface, csv_logger, step_size=500, tolerance=0.000002, max_iterations=100, fake = False):
        self.can = can_interface
        self.csv_logger = csv_logger
        self.step_size = step_size
        self.tolerance = tolerance
